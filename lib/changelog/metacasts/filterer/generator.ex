defmodule Changelog.Metacasts.Filterer.Generator do
  ...
  defp generate_filter(%FacetStatement{repr: repr, logic: logic, items: items}, [look_ahead | ahead_statements] = statements) do
  ...
  defp generate_filter(statement, _) do
    Logger.error("An unrecognized statement was encountered, filter generation failed: #{inspect(statement)}")
  ...
  defp generate_sub_facet_filter({:sub_facet_start, type}, [{:sub_facet_logic, logic} | statements]) do
    {sub_statements, statements, depth} = Enum.reduce(statements, {[], [], 1}, fn statement, {sub, rem, depth} ->
      {sub, rem} = if depth > 0 do
        {[statement | sub], rem}
      else
        {sub, [statement | rem]}
      end

      depth = case statement do
    ...
    if depth > 0 do
      raise FiltererError, {:bad_nesting, "Bad nesting encountered, ended sub-faceting with depth > 1"}
    end

    sub_filters = generate_filters(sub_statements)
    {invert?, enumerator_function} = case {type, logic} do

defmodule Changelog.Metacast do
    ...
    defp validate_filter_query(%{changes: %{filter_query: filter_query}} = changeset) do
        case Cache.compile(filter_query) do
            {:ok, _} -> changeset
            {:error, :no_start} -> add_error(changeset, :filter_query, "Your filter doesn't have a start statement. Start is either 'only' or 'except'.")
            {:error, :too_many_statements} -> add_error(changeset, :filter_query, "Your filter has too many statements. Statement limit: #{Filterer.statement_limit()}")
            {:error, error} -> add_error(changeset, :filter_query, "Error in filter parsing: #{error}")
        end
    end

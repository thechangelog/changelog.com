defmodule Changelog.Github.PullerTest do
  ...
  describe "update" do
    test "fetches show notes for valid episode" do
      ...
      with_mock(HTTPoison, get!: fn _, _, _ -> %{status_code: 200, body: ""} end) do
        Puller.update("show-notes", e)
        assert called(HTTPoison.get!(Source.new("show-notes", e).raw_url, :_, :_))
      end
    end
    ...
    test "fetches multiple transcripts for valid episodes" do
      ...
      with_mock(HTTPoison, get!: fn _, _, _ -> %{status_code: 200, body: ""} end) do
        ...
        assert called(HTTPoison.get!(Source.new("transcripts", e1).raw_url, :_, :_))
        assert called(HTTPoison.get!(Source.new("transcripts", e2).raw_url, :_, :_))
      end
    end
    ...
    test "fetches transcript for valid path" do
      ...
      with_mock(HTTPoison, get!: fn _, _, _ -> %{status_code: 200, body: ""} end) do
        ...
        assert called(HTTPoison.get!(Source.new("transcripts", e).raw_url, :_, :_))
      end
    end
    ...
    test "fetches multiple transcripts for valid paths" do
      ...
      with_mock(HTTPoison, get!: fn _, _, _ -> %{status_code: 200, body: ""} end) do
        Puller.update(
          "transcripts",
          ~w(podcast/the-changelog-292.md podcast/the-changelog-300.md gotime/go-time-bonus-77.md)
        )

        assert called(HTTPoison.get!(Source.new("transcripts", e1).raw_url, :_, :_))
        assert called(HTTPoison.get!(Source.new("transcripts", e2).raw_url, :_, :_))
        assert called(HTTPoison.get!(Source.new("transcripts", e3).raw_url, :_, :_))
      end
    end
    ...
    test "fetches multiple show notes for valid paths" do
      ...
      with_mock HTTPoison, get!: fn _, _, _ -> %{status_code: 200, body: ""} end do
        ...
        assert called(HTTPoison.get!(Source.new("show-notes", e1).raw_url, :_, :_))
        assert called(HTTPoison.get!(Source.new("show-notes", e2).raw_url, :_, :_))
      end
    end
